...
async def get_user(registration: str | None = None, session: Session = Depends(get_current_user)):
    if not registration:
        filter_str = "*"
    else:
...
@router.post("/disable")
async def disable_user(payload: DisableUserRequest, session: Session = Depends(require_editor)):
    ...
    lookup_output = lookup_result.stdout.decode("utf-8", errors="ignore").strip()
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro retornado pelo AD: {lookup_output}"
        )
